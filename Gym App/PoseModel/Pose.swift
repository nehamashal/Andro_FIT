...
struct PoseS {
    ...
    static func fromObservations(_ observations: [Observation]?) -> [PoseS]? {
        // Convert each observations to a `Pose`.
        observations?.compactMap { observation in PoseS(observation) }
    }
    ...
    init?(_ observation: Observation) {
        ...
        //
        area = PoseS.areaEstimateOfLandmarks(landmarks)
