...
class ScanFaceVC: UIViewController, ARSCNViewDelegate, UIImagePickerControllerDelegate ,UINavigationControllerDelegate ,AVCapturePhotoCaptureDelegate{
    ...
    @IBOutlet weak var createFaceBtn:UIButton!
    ...
    override func viewDidLoad() {
        ...
        if categoryIcon == Data() {
            self.view.makeToast("Please Go back to Register your Account",position: .center)
            self.createFaceBtn.isHidden = false
        }else{
            self.createFaceBtn.isHidden = true
            frirstImage.image = UIImage(data: categoryIcon)
        }
        configureDevice()
    }
    @IBAction func actionNewface(_ sender:UIButton){
...
//                let similarity = String(format: "%.2f", firstPair.similarity?.doubleValue ?? 0.0)
//                print("Similarity: \(similarity)")
//                self.percentageLbl.text =  "\(similarity)%"
                let similarity = firstPair.similarity?.doubleValue ?? 0.0
                let roundedSimilarity = Int(similarity * 100)
                self.percentageLbl.text = "\(roundedSimilarity)%"

                    if similarity <= 0.60 {
                        self.view.makeToast("Face not match ",position: .center)
                        self.createFaceBtn.isHidden = false
                    }else{
