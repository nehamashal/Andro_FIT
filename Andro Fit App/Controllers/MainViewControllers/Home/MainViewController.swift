...
//            self.updateSet()
            ...
            if self.index_Ex == 0 {
                    ...
                    self.pushUp_Exercise(bodyParts: bodyParts)
                    
            }else if self.index_Ex == 1 {
                    DispatchQueue.main.async {
                        self.exerciseNameLbl.text = "Squats"
                    }
                    self.squat_Exercise(bodyParts: bodyParts)
                    
            }else if self.index_Ex == 2 {
                    DispatchQueue.main.async {
                        self.exerciseNameLbl.text = "Shoulder Press"
                    }
                    self.shoulderPress_Exercise(bodyParts: bodyParts)
                    
                }else{
                    self.allInOneFucntion(bodyParts: bodyParts)
                }
   ...
   private func drawPoses(_ poses: [PoseS]?, onto frame: CGImage) {
        ...
        let frameWithPosesRendering = poseRenderer.image { rendererContext in
            ...
            for pose in poses {
                ...
                bodyParts = videoProcessingChain.detectedBodyPose()
...
extension MainViewController{
    public func calculateAngle(bodyPart1: CGPoint, bodyPart2: CGPoint, bodyPart3: CGPoint) -> Int? {
...
extension MainViewController {
    ...
    func showSetCount() {
        if set == 0 {
            ...
            if squat_Count % 15 == 0 || pushUp_Count % 15 == 0 || shoulderPressCount % 15 == 0 {
                print("shoulderPressCount \(shoulderPressCount)")
                if hideSetView == "" {
                    hideSetView = "hide"
                    DispatchQueue.main.async {
