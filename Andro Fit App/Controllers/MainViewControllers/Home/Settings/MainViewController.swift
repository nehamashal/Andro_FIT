...
extension MainViewController {
    override func viewDidLoad() {
        ...
        self.animatedView.layer.borderWidth = 5
        self.animatedView.layer.borderColor = UIColor.white.cgColor
...
extension MainViewController {
    ...
    @IBAction func historyBtn(_ sender: UIButton){
        let vc = storyboard?.instantiateViewController(identifier: "WorkoutVideoPlayVC") as! WorkoutVideoPlayVC
//        vc.hideBackBtn = false
        vc.selectedWorkout = index_Ex
...
/// - Down
        /*
         down :  80-130 sq      110-130     0-50    0-90       110 -140
                ...
                60-160       40-90       30-90      100-180     60-160
         */
         ...
         if (hip_AngleR > 160 || hip_AngleL > 160) {
         ...
         }else if (hip_AngleR < 140) || (hip_AngleL < 140) {
             if (shoulder_AngleR >= 80 && shoulder_AngleR <=  150) || (shoulder_AngleL >= 80 && shoulder_AngleL <=  150) {
                 ...
                 if (shoulder_AngleR >= 40 && shoulder_AngleR <= 90) || (shoulder_AngleL >= 40 && shoulder_AngleL <= 90) {
...
/// -  Middle
        /*
         middle:    130-160     110-130    0-50       0-90      140-160
                    60-160       90-140      50-100     100-180     60-160
         ...
         */
         ...
         }else if (hip_AngleR > 140 && hip_AngleR <= 160) || (hip_AngleL > 140 && hip_AngleL <= 156) {
             if (shoulder_AngleR >= 80 && shoulder_AngleR <= 150) || (shoulder_AngleL >= 80 && shoulder_AngleL <= 100) {
                if (elbow_AngleR < 50 || elbow_AngleL < 50) {
                    if (angle_HEW_R <= 90 || angle_HEW_L <= 90) {
                        if (knee_AngleR >= 100 && knee_AngleR <= 160) || (knee_AngleL >= 100 && knee_AngleL <= 160) {
                            ///  squat
 ...
 /// -  up
        /*
         up :
                60-160      140-180     100-180    100-180      60-160
         ...
         */
         if (hip_AngleR > 160 || hip_AngleL > 160) {
                if (shoulder_AngleR >= 80 && shoulder_AngleR <= 150) || (shoulder_AngleL >= 80 && shoulder_AngleL <= 150) {
                    if (elbow_AngleR <= 50 || elbow_AngleL <= 50) {
                        if (angle_HEW_R <= 90 || angle_HEW_L <= 90) {
                            if (knee_AngleR > 100 || knee_AngleL > 100) {
                                /// squat
                                DispatchQueue.main.async {
                                    self.actionLabel.text = "\(self.squat_Count)"
...
//  user in a down position
                            ...
                            if properCount_Sq == "1" {
                                squat_Count += 1
                                    DispatchQueue.main.async {
                                        self.actionLabel.text = "\(self.squat_Count)"
                                        self.set = self.squat_Count / 15
                                        self.setOfExercise.text = "\(self.set)/\(self.squat_Count)"
                                    }
                                    countStore()
...
//  user's arms in an upper position
                            DispatchQueue.main.async {
                                self.actionLabel.text = "\(self.pushUp_Count)"
 ...
 //  user's arms in a down position
                            if self.properCount_PU == "1" {
                                self.pushUp_Count += 1
                                    DispatchQueue.main.async {
                                        self.actionLabel.text = "\(self.pushUp_Count)"
                                        self.set = self.pushUp_Count / 15
                                        self.setOfExercise.text = "\(self.set)/\(self.pushUp_Count)"
                                    }
                                    countStore()
...
//  user's arms in down position
                        ...
                        DispatchQueue.main.async {
                            self.actionLabel.text = "\(self.shoulderPressCount)"
                            self.set = self.shoulderPressCount / 15
                            self.setOfExercise.text = "\(self.set)/\(self.shoulderPressCount)"
...
extension MainViewController {
    func updateSet() {
        ...
        // Check if the current set is divisible by 15
        if set % 15 == 0 {
    ...
    func showSetCount() {
        if self.set == 0 {
            ...
            if self.squat_Count % 15 == 0 || self.pushUp_Count % 15 == 0 || self.shoulderPressCount % 15 == 0 {
                print("shoulderPressCount \(self.shoulderPressCount)")
                if hideSetView == "" {
                    self.hideSetView = "hide"
                    DispatchQueue.main.async {
