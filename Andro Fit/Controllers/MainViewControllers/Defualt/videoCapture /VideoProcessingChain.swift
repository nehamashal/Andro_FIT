...
extension VideoProcessingChain {
    ...
    private func findPosesInFrame(_ frame: CGImage) -> [PoseS]? {
        ...
        let poses = PoseS.fromObservations(humanBodyPoseRequest.results)
        
        //detectedBodyPose(request: humanBodyPoseRequest)
    ...
    func detectedBodyPose() -> [VNHumanBodyPoseObservation.JointName : VNRecognizedPoint]{
        
            guard let bodyPoseResults = humanBodyPoseRequest.results
              else { return [VNHumanBodyPoseObservation.JointName : VNRecognizedPoint]()}
            guard let bodyParts = try? bodyPoseResults.first?.recognizedPoints(.all) else { return [VNHumanBodyPoseObservation.JointName : VNRecognizedPoint]()}
            DispatchQueue.main.async {
    ...
    private func isolateLargestPose(_ poses: [PoseS]?) -> PoseS? {
    ...
    private func multiArrayFromPose(_ item: PoseS?) -> MLMultiArray? {
    ...
    private func predictActionWithWindow(_ currentWindow: [MLMultiArray?]) -> ActionPrediction {
        ...
        let filledWindow: [MLMultiArray] = currentWindow.map { multiArray in
            if let multiArray = multiArray {
                ...
                return PoseS.emptyPoseMultiArray
            }
        }
